...
function evidenceQuality(evidence: LearningEvidence | null, dimension: EvidenceDimension) {
  ...
  if (dimension === "rebuild") {
    return Math.max(completionBase, attemptQuality(evidence));
  }

  if (dimension === "explain") {
    return Math.max(completionBase, explanationQuality(evidence));
  }

  if (dimension === "transfer") {
    return Math.max(completionBase, attemptQuality(evidence), recognitionQuality(evidence));
  }
